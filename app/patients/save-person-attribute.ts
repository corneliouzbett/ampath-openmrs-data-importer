...
export async function savePersonAttributes(patient:PatientData, insertMap:InsertedMap, AmrsConnection: Connection) {
    ...
    if (userMap) {
        replaceColumns = {
            ...
            person_attribute_type_id:attributeTypeMap[attribute.person_attribute_type_id]
        };
    }
    await CM.query(toPatientAttributeInsertStatement(attribute, replaceColumns), AmrsConnection);
}
}

export function toPatientAttributeInsertStatement(attribute: PersonAttribute, replaceColumns?: any) {
    return toInsertSql(attribute, ['person_attribute_id'], 'person_attribute', replaceColumns);
}